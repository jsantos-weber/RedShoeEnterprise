...
CREATE VIEW RSE_Current_Employees AS
    SELECT 
            ...
            CASE 
                ...
                ELSE
                    CONVERT(varchar(2),
                            (Select (B.Salary / C.Salary * 100)- 100
                                FROM EmployeeJobs B 
                                JOIN 
                                (
                                    SELECT A.EmployeeJobKey, A.EmployeeKey, A.Salary--, D.FirstName, D.LastName
                                    FROM dbo.EmployeeJobs A JOIN dbo.Employees D ON(A.EmployeeKey = D.EmployeeKey)
                                    RIGHT JOIN dbo.EmployeeJobs C ON(A.EmployeeJobKey = C.EmployeeJobKey)
                                    WHERE A.Salary < 
                                    (
                                        SELECT max(B.salary) 
                                        FROM dbo.EmployeeJobs B 
                                        Where A.EmployeeKey = B.EmployeeKey
                                    )
                                GROUP BY A.EmployeeJobKey, A.EmployeeKey, A.Salary
                            ) AS C ON (C.EmployeeKey = B.EmployeeKey)
                            WHERE 
                                B.JobFinish is null),
                    20) + '%'
